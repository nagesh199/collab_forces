      ...
      <div className={styles.lossers}>
        {loseData.map((item) => (
          <div style={{ fontFamily: "sans-serif" }} key={item._id}>
            ...
            />
            <p style={{ marginLeft: "5%", fontSize: "13px", marginLeft: "5%" }}>
              {item.title}
            </p>
            <p style={{ color: "red", fontSize: "12px", marginLeft: "5%" }}>
              ₹ {item.current - count.toFixed(2)}
            </p>
            <div
              style={{
                display: "flex",
            ...
            >
              <p style={{ color: "red", fontSize: "12px" }}>
                -{Math.abs(item.basePrice - item.current - count).toFixed(2)}
              </p>
              <p style={{ color: "red", fontSize: "12px" }}>
                (
                {(
                  (Math.abs(item.basePrice - item.current - count) /
                    item.current) *
                  100
                ).toFixed(2)}{" "}
                %)
